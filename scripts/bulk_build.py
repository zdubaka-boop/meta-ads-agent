...
def settings_from(campaign_id):
    ...
    if sets_:
        ...
        out.update({
            ...
            "publisher_platforms": t.get("publisher_platforms"),
...
def main():
    ...
    for gname, files in groups.items():
        ...
        for f in files:
            ang = angle_of(f.name)
            c = (gcopy.get(ang) if ang and isinstance(gcopy.get(ang), dict) else None) \
                or gcopy.get("_default") \
                or (gcopy if ("bodies" in gcopy or "headlines" in gcopy) else {})
